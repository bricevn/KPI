...
(function () {
  ...
  window.TabCalendar = function TabCalendar() {
    ...
    let rows = A.detail.filter(function (d) { return Object.keys(d.seg).length > 0; });
    ...
    return (
      <React.Fragment>
        <div className="cal-toolbar">
          <span className="muted" style={{ fontSize: 12, fontWeight: 600 }}>Phases</span>
          <window.InfoTip text="Une ligne par issue, positionnée sur la timeline de la milestone. Cliquez sur une phase pour trier par sa date de début, sur l’œil pour la masquer. Le trait vertical marque aujourd’hui." />
          <div className="cal-legend" style={{ marginLeft: 6 }}>
            ...
            <span key={k} className={'cal-lg' + (hidden.has(k) ? ' off' : '') + (s.key === k ? ' sort' : '')}>
                ...
                <button className="cal-eye" title="Masquer / afficher" onClick={() => toggle(k)}><Eye off={hidden.has(k)} /></button>
              </span>
            )}
          </div>
          <Nav />
          ...
          <div className={'gantt-scroll gantt-drag' + (dragging ? ' grabbing' : '')} ref={scrollRef}>
            <div className="gantt-grid" style={gridStyle}>
              <div className="gantt-axis">
                <div className="gantt-axis-corner">Issue</div>
                {Array.from({ length: A.cal.WEEKS }, (_, i) => <span key={i} className="wk">S{i + 1}</span>)}
              </div>
              {rows.map((d) =>
        ...
        <div className="muted" style={{ fontSize: 11.5, marginTop: 8, display: 'flex', gap: 18 }}>
          <span><span style={{ display: 'inline-block', width: 14, height: 2, background: 'var(--accent)', verticalAlign: 'middle', marginRight: 5 }}></span>Aujourd’hui ({A.cal.fmtDay(TODAY)})</span>
        </div>
      </React.Fragment>);
