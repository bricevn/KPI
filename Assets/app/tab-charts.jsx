...
(function () {
  ...
  function agg(typeKeys) {
    ...
    const o = {};
    PH.forEach(({ k }) => {o[k] = sum((r) => r[k] * r.issues) / (issues || 1);});
    return {
  ...
  function TypeBreakdown({ typeKeys }) {
    return (
      <table className="tbl">
        <thead><tr><th>Type</th><th className="num">Issues O/F</th><th className="num">Poids V/T</th><th className="num">Approvals F/N</th><th className="num">Temps</th></tr></thead>
        <tbody>
          {typeKeys.map((k) => {
  ...
  function RecapSection({ style }) {
    ...
    <div className="gm-list">
        <MetricBar label="Avancement" value={d.closed} total={d.issues} color="var(--c-done)" />
        <MetricBar label="Poids validé" value={d.wV} total={d.wV + d.wNV} color="var(--c-good)" />
        <MetricBar label="Approvals" value={d.appr} total={d.issues} color="var(--c-regression)" />
        <MetricBar label="Temps (lead)" pct={Math.min(100, d.lead / 18 * 100)} suffix={d.lead.toFixed(1) + ' j'} color="var(--p-qawait)" />
      </div>;

    return (
      <div className="g-section">
        <div className="g-sec-h"><h3>Récap par super-groupe</h3><window.InfoTip text="Issues, poids, approvals et temps regroupés par famille (Features, Bugs & Régression). Cliquez un groupe pour le détail par Type::*." /></div>
        ...
        <div className="sg-cards">
          ...
          <button key={g.key} className="sg-card" onClick={() => openDrill(g)}>
                ...
                <div className="sg-card-h"><span className="dot" style={{ background: g.color }}></span><span className="sg-name">{g.name}</span><span className="sg-share" title="Part de ce groupe dans le poids total du périmètre">{shareOf(g.d)}% du total</span></div>
                <div className="sg-issues"><b>{g.d.issues}</b> issues · <b>{g.d.closed}</b> fermées · <b>{g.types.length}</b> types</div>
                <Metrics d={g.d} />
              </button>
          )}
          </div> :
        ...
        <div className="pnl"><div className="tbl-scroll"><table className="tbl big">
            <thead><tr><th>Super-groupe</th><th className="num">Issues O/F</th><th className="num">Poids V/T</th><th className="num">Approvals F/N</th><th className="num">Temps</th><th className="num" title="Part dans le poids total du périmètre">Part</th><th></th></tr></thead>
            <tbody>
              {groups.map((g) =>
        ...
        {drill && <window.Modal title={drill.name} subtitle="Détail par Type::*" headline={<React.Fragment>{drill.d.issues} issues<span className="mh-share">{shareOf(drill.d)}% du total</span></React.Fragment>} wide layout={typeof window !== 'undefined' && window.__drillLayout || 'modal'} onClose={() => setDrill(null)}>
          <div className="dm-metrics">
            <div className="dm-kpi"><span className="dm-v" style={{ color: 'var(--c-done)' }}>{window.pctOf(drill.d.closed, drill.d.issues)}%</span><span className="dm-l">avancement</span></div>
            <div className="dm-kpi"><span className="dm-v" style={{ color: 'var(--c-good)' }}>{window.pctOf(drill.d.wV, drill.d.wV + drill.d.wNV)}%</span><span className="dm-l">poids validé</span></div>
            <div className="dm-kpi"><span className="dm-v" style={{ color: 'var(--c-regression)' }}>{window.pctOf(drill.d.appr, drill.d.issues)}%</span><span className="dm-l">approvals</span></div>
            <div className="dm-kpi"><span className="dm-v" style={{ color: 'var(--p-qawait)' }}>{drill.d.lead.toFixed(1)} j</span><span className="dm-l">temps (lead)</span></div>
          </div>
  ...
  function PoidsSection({ style }) {
    ...
    <div className={'g-legend' + (hi ? ' has-hi' : '')}>
        ...
        <span {...legProps('nv')}><span className="sw hatch"></span>non validé</span>
      </div>;

    return (
      <div className="g-section">
        <div className="g-sec-h"><h3>Poids par valeur &amp; par type</h3><window.InfoTip text="Répartition des points (1·2·3·5·8·13) sur les types, avec la part validée (pleine) et non validée (hachurée)." /></div>
              ...
              <div key={w} className="pw-row">
                ...
                <span className="pw-stack" style={{ width: totalAt(wi) / maxRow * 100 + '%' }}>
                  {types.map((tk) => {
                    ...
                    <i style={{ width: c.v / tot * 100 + '%', background: window.typeColor(tk), opacity: vOp(tk), transition: 'opacity .15s' }} title={`${A.typeByKey[tk].short} · ${w} pts · ${c.v} validé`}></i>
                    <i className="hatch-seg" style={{ width: c.nv / tot * 100 + '%', background: window.typeColor(tk), opacity: nvOp(tk), transition: 'opacity .15s' }} title={`${A.typeByKey[tk].short} · ${w} pts · ${c.nv} non validé`}></i>
                  </React.Fragment>;
                  })}
                </span>
        ...
        <div className="pnl"><div className="tbl-scroll"><table className="pw-matrix">
            <thead><tr><th>Type \ Poids</th>{FIB.map((w) => <th key={w} className="num">{w}</th>)}<th className="num">Σ</th></tr></thead>
            <tbody>
              {types.map((tk) =>
                <tr key={tk}>
  ...
  function TempsSection({ style }) {
    ...
    return (
      <div className="g-section">
        <div className="g-sec-h"><h3>Temps moyen par phase &amp; par type</h3><window.InfoTip text="Durée moyenne (jours) passée dans chaque phase, déclinée par Type::* — extension du bloc « Temps moyen par phase » du Dashboard." /></div>
        ...
        <div className="pnl"><div className="pnl-b"><Legend />
          <div className="tt-stacked">
            {types.map((tk) => {
              ...
              <span className="tt-lead">{lead.toFixed(1)} j</span>
            </div>;
              })}
          </div></div></div>
        }
        ...
        {style === 'matrice' &&
        <div className="pnl"><div className="tbl-scroll"><table className="pw-matrix">
            <thead><tr><th>Type \ Phase</th>{PH.map(({ pk }) => <th key={pk} className="num">{window.PHASE_NAME[pk]}</th>)}<th className="num">Lead</th></tr></thead>
            <tbody>
              {types.map((tk) => {
