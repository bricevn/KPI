  ...
  function IssueLine({ d }) {
    return (
      <div className="arow">
        ...
        <span className={'st-badge ' + (d.state === 'closed' ? 'st-closed' : 'st-open')}>{d.state === 'closed' ? window.t('common.closedF') : window.t('common.openF')}</span>
      </div>);
  }
          ...
          {CATS.map((c) => {
            ...
            return (
              <button key={c.key} className={'acard' + (open === c.key ? ' on' : '')} onClick={() => setOpen(c.key)}>
                ...
                <span className="acard-t">{window.t('an.' + c.key)}</span>
              </button>);
          })}
        </div>
            ...
            return (
              <div key={c.key} className="pnl alist">
                <button className="ahd" onClick={() => setOpen(isOpen ? '' : c.key)}>
                  ...
                  <span className="ahd-title">{window.t('an.' + c.key)}</span>
                  <span className="ahd-desc">{window.t('an.' + c.key + 'D')}</span>
                  <span className="ahd-count">{list.length}</span>
                  <span className={'ahd-chev' + (isOpen ? ' open' : '')}>{window.ICONS.chevron}</span>
                </button>
                {isOpen && (list.length ? <div className="alist-body">{list.map((d) => <IssueLine key={d.iid} d={d} />)}</div> : <div className="empty">{window.t('an.none')}</div>)}
              </div>);
          })}
        </div>
      </React.Fragment>);
